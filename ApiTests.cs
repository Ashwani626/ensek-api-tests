...
namespace ENSEK_Test
{
    ...
    public class ApiTests
    {
        ...
        public void BuyEachFuel()
        {
            ...
            {
                var request = new RestRequest($"/buy/{fuel}/10", Method.Post);
                var response = _client.Execute(request);
                response.StatusCode.Should().Be(System.Net.HttpStatusCode.OK);
            }
        }
        ...
        public void VerifyOrdersExist()
        {
            ...
            foreach (var fuel in _fuels)
            {
                orders.Should().Contain(o => o.Fuel.Equals(fuel, StringComparison.OrdinalIgnoreCase) && o.Quantity == 10);
            }
        ...
        [Test, Order(5)]
        public void NegativeFuelBuyTest()
        {
            var request = new RestRequest("/buy/unknownfuel/10", Method.Post);
            var response = _client.Execute(request);
